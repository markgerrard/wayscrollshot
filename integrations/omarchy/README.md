...
The menu runs `~/.local/bin/wayscrollshot`, so install the current release build first. It offers All-in-One, Area, Fullscreen, Window, automatic Scrolling capture, and View Cloud. It closes before launching a capture or gallery, keeping the menu out of the image. Keyboard navigation and Escape use Omarchy's native panel controls.
